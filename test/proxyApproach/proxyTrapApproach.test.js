...
suite('Multiple Prototype Chain creation', () => {
  ...
  suite.only('Proxy traps tests:', () => {
    suite('instanceof', () => {
      let { proxy } = new MultipleDelegation()
      let isInstanceOf = proxy instanceof MultipleDelegation
      // regular item equality check of two arrays
      test('instance check returning correctly', () => assert(isInstanceOf, `• proxy should be considered an instance of MultipleDelegation"`))
    })
    ...
    suite('getOwnPropertyDescriptor trap', () => {
      ...
      // regular item equality check of two arrays
      test('only string properties must be returned', () =>
        assert(keyList.every(item => resultFixture.includes(item)) && resultFixture.every(item => keyList.includes(item)), `• returned ownKeys must include string names only"`))
    ...
    suite.only('has trap', () => {
      ...
      test('Should check non existing properties:', () => assert(!Reflect.has(proxy, 'nonExistingKey'), `• nonexisting property check failed`))
    })
  })
