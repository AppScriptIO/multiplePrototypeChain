...
exports.MultiplePrototypeChain = void 0;
let self = class MultiplePrototypeChain {
  static newChainOnInstanceCreation({
    Class,
    // wrap constructor of Class and create a new chain for each new instance.
    ...
    return new Proxy(Class, {
      construct: (target, argumentsList, newConstructorFunc) => {
        ...
        instance = self.createUniqueProtoChain({
          object: instance
        });
  ...
  static createUniqueProtoChain({
    ...
    let nextPointerPrototype = self.createUniqueProtoChain({
      object: delegatedPrototype
    });
    ...
    let proxyHandler = self.handlerMultiplePrototypeChainPattern({
      delegatedPrototype: object
    });
    pointerPrototype = new Proxy(pointerPrototype, proxyHandler);
    return pointerPrototype;
  }

  static handlerMultiplePrototypeChainPattern({
    delegatedPrototype
  }) {
    return {
